...
public class Main {
    public static void main(String[] args) throws MarkOutOfBoundsException, SubjectNotFoundException,
            StudentNotFoundException, GroupNotFoundException, FacultyNotFoundException {
        String[] subjects1 = {"ИКГ", "ИПЭ", "Физика", "ЭТТ"};
        String[] subjects2 = {"ВМиП", "ОАиП", "ИТАС", "ТОЭ"};
        String[] subjects3 = {"ВТвИ", "ОАиП", "Физика", "Математика"};

        //second parameter is an average mark for each subject respectively
        Student student1 = new Student(subjects1, new double[]{5.5, 6.0, 8.9, 4.9});
        Student student2 = new Student(subjects1, new double[]{6.5, 7.0, 5.9, 8.9});
        Student student3 = new Student(subjects2, new double[]{6.2, 7.3, 5.5, 8.0});
        ...
        Group group2 = new Group(student3, student4, student5, student6);
        ...
        try {
            new University();
        } catch (FacultyNotFoundException e) { e.printStackTrace(); }
        try {
            new Faculty();
        } catch (GroupNotFoundException e) { e.printStackTrace(); }
        try {
            new Group();
        } catch (StudentNotFoundException e) { e.printStackTrace(); }
        try {
            new Student(new String[]{"ИКГ"}, new double[]{-5});
        } catch (SubjectNotFoundException e) { e.printStackTrace(); }
